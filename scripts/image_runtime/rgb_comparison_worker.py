"""One root-authorized RGB reference comparison; original edit evidence retained."""
...
with m.acquire_lease(blocking=False):
 ...
 rel='receipts/'+rid+'-rgb-comparison-telemetry.jsonl';start=time.monotonic()
 try:
  with r.anchor() as a:
   with a.open(rel,os.O_WRONLY|os.O_CREAT|os.O_EXCL) as output:
    ...
    try:
     ...
     reply=m.run(['docker','exec',cid,'/opt/image-venv/bin/python','-I','-B','/work/evidence/'+rid+'/rgb-comparison/request.py'],timeout=820)
  ...
  with r.anchor() as a:a.atomic_json('receipts/'+rid+'-rgb-comparison.json',receipt)
  s.update(rgb_comparison=receipt);r.save(s);r.guards();print(json.dumps(receipt),flush=True)
  ...
  s.update(phase='failed',warm=False,rgb_comparison_failure={'type':type(error).__name__,'code':str(error) if type(error) is RuntimeError else 'edit_failed','utc':m.now(),'telemetry_path':str(BASE/rel)})

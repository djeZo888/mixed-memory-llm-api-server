#!/usr/bin/env python3
"""Ubuntu 24.04 installer source, I1 prerequisites boundary. Full install pending I1b."""
...
from install.core import InstallError, Pending, Runner, State, atomic_json, digest, exclusive, private_path, read_private_json
from install.host import budget, model_plan, observe, supported

BOOTSTRAP = Path("/etc/local-ai-server/bootstrap.json")
STAGES = ["preflight", "storage", "base", "driver", "container", "runtime", "models", "service", "client", "acceptance"]
IMPLEMENTED = {"preflight", "storage", "base", "driver"}
...
def parser():
    ...
    p.add_argument("--through", choices=("storage", "base", "driver"), help="Explicit partial prerequisites boundary; never READY")
...
def make_plan(config, args, runner):
    ...
    return {"schema_version": 1, "evidence_class": "SYNTHETIC_FIXTURE" if args.fixture_host else "READ_ONLY_DISCOVERY",
            "installer_status": "INCOMPLETE_I1B_REQUIRED", "ready": False, "host": host,
            "host_supported": supported(host), "config": config, "storage": storage, "models": models,
            "disk_budget": disk_budget, "version_lock": lock, "lock_hash": lock_hash, "input_hash": source_identity(),
            "stages": [{"name": s, "implementation": "available" if s in IMPLEMENTED else "pending_I1b"} for s in selected],
            "pending_required_stages": pending,
            "effects": ["plan writes nothing", "full apply refused before mutation until required stages integrated",
                "explicit --through selects partial prerequisites only", "driver changes may require manual reboot; exit75 then resume",
            ...
            "limitations": ["blank disk mutation pending I1b", "L1 generic lifecycle and F1S safe auth gate required",
...
def run_boundary(config, args, runner, plan, lock_hash):
    ...
    if config["storage_mode"] == "initialize":
        raise Pending("blank_disk_mutation_requires_i1b")
    # Shared with D2 today; I1b must use L1's same-lease dispatch contract.
...
def main(argv=None):
    ...
    try:
        ...
        if args.command == "status":
            from install.storage import Storage
            Storage(config, runner).guard()
            state_path = Path(config["data_dir"]) / "services/installer/state.json"
            read_private_json(state_path)
            _, lock_hash = version_lock()
            state = State(state_path, config, lock_hash, source_identity(), lambda: Storage(config, runner).guard()).value
            print(json.dumps({"schema_version": 1, "ready": False, "status": "PARTIAL_PREREQUISITES", "recorded": state,
        ...
        if not args.through or config["role"] == "client":
            raise Pending("required_deployment_stages_not_implemented_i1b")

#!/usr/bin/env python3
"""Control one exact I2R hosted workflow run using existing gh authentication."""
import argparse
import io
import json
import os
...
WORKFLOW = ".github/workflows/i2r-linux.yml"
...
def validate_run(data, run_id, head):
    if (data.get("id") != run_id or data.get("head_sha") != head or
            ...
            data.get("head_branch") != "milestone/i2r-package-linux" or
...
def main():
    ...
    if args.action == "stop":
        ...
        expected = f"i2r-evidence-{args.run_id}-{data['run_attempt']}"
        ...
        manifest = json.loads((args.destination / "source-manifest.json").read_text())
        if manifest.get("source_sha") != args.head:
            raise RuntimeError("artifact_manifest_source_mismatch")
        print(json.dumps({"artifact_id": artifact["id"], "digest": artifact.get("digest"),
                          "source_sha": args.head, "status": evidence.get("status"),
                          "collected": "sanitized_evidence"}, sort_keys=True))

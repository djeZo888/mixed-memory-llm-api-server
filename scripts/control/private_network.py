...
EXPECTED = {
    ...
    'ports': {'control': 30000, 'glm': 30002, 'qwen38': 30004, 'image': 30006, 'node': 30008, 'frontier': 30010},
}
...
def expected_units():
    ...
    units = {}
    for role, port in (("control", 30000), ("glm", 30002), ("qwen38", 30004), ("image", 30006), ("node", 30008), ("frontier", 30010)):
...
def _units_stopped():
    for name in expected_units():
        ...
        _require(info.get('ActiveState') in ('inactive', 'failed')
                 ...
                 'stop all twelve transport units and disable sockets before ingress removal')
...
def _rules():
    jump = ['-d', '10.156.100.60/32', '-p', 'tcp', '-m', 'multiport', '--dports',
            '30000,30002,30004,30006,30008,30010', '-m', 'comment', '--comment', TAG, '-j', CHAIN]
    rules = [
...
def source_check():
    ...
    for name, content in expected_units().items():
        _require((root / 'configs/network' / name).read_bytes() == content.encode(),
                 'source unit drift')
    return 'fixed source policy and twelve exact units passed (systemd runtime NOT_TESTED)'

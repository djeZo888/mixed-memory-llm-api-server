...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='q38b-git-source-') as directory:
        ...
        result = subprocess.run([sys.executable, '-B', '-m', 'unittest',
            ...
            'tests.lifecycle.test_qwen38_final_source', 'tests.lifecycle.test_qwen38_native_wire',
            'tests.test_control_installation', '-v'], cwd=directory,
            stdin=subprocess.DEVNULL, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
            timeout=60, check=False)
    print(json.dumps({'commit': commit, 'tree': tree,
        'status': 'PASS' if result.returncode == 0 else 'FAIL',
        'checks': 'committed-source receipt happy path, byte drift, native wire source controls and exact control closure/protection',

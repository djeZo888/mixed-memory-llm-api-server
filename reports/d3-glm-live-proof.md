...
No tuning variant was selected. Existing shard1 metadata contains zero tensor descriptors, so per-layer expert bytes are unavailable. [Exact read-only metadata follow-up](d3t-metadata-follow-up.md) specifies the bounded next step. Snapshot unallocated VRAM is81,083/84,947MiB before V1 peak/workspace reserves; layer-split late experts may concentrate on one GPU. No new live probe or reload was made.

...
## Authorized same-session follow-up at04:38 UTC

The later [device-name observation](q38retry-device-names.md) identifies the rejected entry as root-owned directory `/dev/nvidia-caps`,0755,rdev0:0. It first appears between the auth-failure child's last parser/template snapshot and its exit, persists into the timeout child and is refused by that child's entry isolation check. The earlier unknown-name statements above describe evidence available at04:29; their original observations remain unchanged. Exact creator and directory contents remain unobserved. No policy/source correction or auth/model acceptance followed. Follow-up cleanup/postguards passed and ownership released04:38:07.406178Z.

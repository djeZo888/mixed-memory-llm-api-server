...
NAME='llm-image21-reference-corrected-20260923'
...
with m.acquire_lease(blocking=False):
 ...
 with anchor(BASE) as a:
  a.mkdir('corrected',mode=0o755)
  assert not (BASE/'corrected/trial.json').exists(),'single trial already recorded'
  a.atomic_json('corrected/trial.json',summary)
...
with m.acquire_lease(blocking=False):
 ...
 with anchor(BASE) as a:
  a.mkdir('corrected/work',mode=0o755)
  a.mkdir('corrected/work/tmp',mode=0o700);a.mkdir('corrected/work/cache',mode=0o700);a.mkdir('corrected/work/tmp/cccccccccccccccccccccccccccccccc',mode=0o700)
  for p in (BASE/'corrected/work',BASE/'corrected/work/tmp',BASE/'corrected/work/cache',BASE/'corrected/work/tmp/cccccccccccccccccccccccccccccccc'):os.chown(p,1000,1001)
  a.check()
  argv=['docker','create','--name',NAME,'--label','io.llm-reference.owner='+TASK,'--label','io.llm-reference.session='+SESSION,'--gpus','device='+GPU,'--network','none','--user','1000:1001','--cpuset-cpus','8-15','--cpuset-mems','0','--memory','96g','--memory-swap','96g','--pids-limit','1024','--shm-size','1g','--restart','no','--log-driver','none','--read-only','--cap-drop','ALL','--security-opt','no-new-privileges','--mount','type=bind,src='+r.model+',dst=/models,readonly','--mount','type=bind,src='+str(BASE/'source')+',dst=/reference,readonly','--mount','type=bind,src='+str(BASE/'corrected/work')+',dst=/work','--mount','type=bind,src='+str(BASE/'corrected/work/tmp')+',dst=/tmp']
  # Copy every approved runtime override directly from its validated installed source.
  original_argv=r.create_argv('cccccccccccccccccccccccccccccccc')
  env={original_argv[i+1].split('=',1)[0]:original_argv[i+1].split('=',1)[1] for i,x in enumerate(original_argv) if x=='--env'}
  env['TOKENIZERS_PARALLELISM']='false'
  summary['effective_environment']=env
  summary['correction_authority']='ROOT-CACHE-CORRECTION-GO.md'
  summary['prior_attempt']={'pipeline_call_counter_assigned_before_call':1,'denoising_steps_completed':0,'output_images':0,'preserved_marker':str(BASE/'trial.json')}

  for k,v in env.items():argv.extend(['--env',k+'='+v])
  argv.extend([IMAGE,'-I','-B','/reference/corrected_entry.py'])
  summary['launch_argv']=argv
  try:
   cid=m.run(argv).stdout.strip();assert len(cid)==64
   summary.update(status='created',container_id=cid,image_id=IMAGE);a.atomic_json('corrected/trial.json',summary);owned()
   with a.open('corrected/telemetry.jsonl',os.O_WRONLY|os.O_CREAT|os.O_EXCL) as telemetry:
    sampler=subprocess.Popen(['/usr/bin/python3','-I','-B',str(m.BASE/'source/telemetry.py'),'--output-fd',str(telemetry.fileno()),'--output-path',str(BASE/'corrected/telemetry.jsonl'),'--cgroup','/sys/fs/cgroup/system.slice/docker-'+cid+'.scope'],pass_fds=(telemetry.fileno(),),stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
    try:
     ...
     with a.open('corrected/trial.log',os.O_WRONLY|os.O_CREAT|os.O_EXCL) as log:
      summary.update(status='running',start_monotonic=time.monotonic(),start_utc=m.now());a.atomic_json('corrected/trial.json',summary)
   ...
   records=[json.loads(x) for x in (BASE/'corrected/telemetry.jsonl').read_text().splitlines()]
   ...
   summary['finish_utc']=m.now();a.atomic_json('corrected/trial.json',summary);r.guards()

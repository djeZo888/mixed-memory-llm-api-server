...
with m.acquire_lease(blocking=False):
 prior=json.loads((BASE/'corrected/trial.json').read_text())
 ...
 receipt={'utc':m.now(),'session_id':SESSION,'status':'CORRECTED_VISUAL_FAIL_SGLANG_RESTORED_WARM','qwens_unchanged':True,'qwens':qwens(),'sglang':resident,'sglang_image_id':r.config['image_id'],'sglang_run_id':state['run_id'],'sglang_unit':m.run(['systemctl','show',m.UNIT,'-p','ActiveState','-p','SubState','-p','UnitFileState']).stdout,'reference_container':{'id':trial['Id'],'state':trial['State']},'reference_gpu_context_released':True,'recovery_generation_count':1,'corrected_reference_comparisons':1,'additional_reference_attempts_after_correction':0,'retained_reference_image_id':prior['image_id'],'source_sha256':{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in (BASE/'source').iterdir() if p.is_file()},'gpu_processes':m.run(['nvidia-smi','--query-compute-apps=gpu_uuid,pid,used_memory','--format=csv,noheader,nounits']).stdout}
 r.guards()
receipt['canonical_lease_released']=True
with anchor(BASE) as a:a.atomic_json('corrected/final-verification.json',receipt)

...
## Outcome: PARTIALLY CACHED baseline retained; no more requests

One authorized cold request completed; the shipped probe rejected it because the backend reported **50 cached tokens**, while the cold baseline requires exactly **0**. Native accounting and response usage agree at **1,370 input tokens**, all three retrieval checks passed, and the response ended with `finish_reason=stop`. The shipped zero-cache qualification failed. Root's latest coordination accepts this completed request as a **PARTIALLY CACHED baseline** for later candidate comparison using identical body hashes and native timings, with cache differences disclosed. It does not establish a model failure or a completed four-request matched comparison.
...
**Accepted continuation evidence:** root now accepts this completed **partially cached** request for later candidate comparison; do not retry to obtain zero cache or invent another harness. Compare the exact preserved cold body/hash and native timing/accounting against the later patched N76 configuration, explicitly disclosing baseline cached50/evaluated1320 and the candidate's actual cache/evaluated counts. Warm/tool/continuation remain NOT_TESTED. **Shipped integration gap:** there is no reset/reconcile command, and `bind candidate`, `prepare` and `start` cannot advance this stopped checkpoint because the source still requires baseline PASS. Record this gap for the later authorized source/continuation owner; do not edit state or bypass the contract. Preserve the checkpoint/key and all frozen bytes. Root's evidence acceptance does not manufacture STAGE_PASS or authorize source/lifecycle work in D3BASE. No model switch or further request is performed.

## Verification and artifacts

PASS: source ancestry/hashes; exact baseline admission; canonical and registered guards before/after; actual native template/token accounting; native/usage agreement; retrieval checks;112 sampled safety checks; unchanged host/state/key verification; completion/quiescence reconciliation; owned tunnel cleanup. **Shipped gate STOP:** cold cache50 versus required0. **Coordinator acceptance:** preserve as PARTIALLY CACHED baseline for comparison with cache differences disclosed. All later requests and phases remain NOT_TESTED.

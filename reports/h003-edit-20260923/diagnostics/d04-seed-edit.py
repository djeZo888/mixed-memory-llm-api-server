"""D04 one seed-collision discriminator; original42 regression remains failed."""
...
fields={'model':'qwen-image-2.1','prompt':'Change the red teapot to blue, keeping its shape, table, window, and lighting unchanged.','n':1,'size':'1024x1024','num_inference_steps':40,'guidance_scale':1.0,'true_cfg_scale':1.0,'seed':43,'generator_device':'cpu','output_format':'png','response_format':'b64_json','background':'opaque','enable_teacache':False,'perf_dump_path':str(out/'perf.json')}
(out/'request.json').write_text(json.dumps(fields,indent=2)+'\n')
body,ctype=n.multipart(fields,raw,'h003seed43')
m={'id':'D04','http_calls':0,'completed_outputs':0,'steps_completed':None,'status':'prepared'}

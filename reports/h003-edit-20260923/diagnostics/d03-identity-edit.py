"""D03 one identity-edit request; diagnostic only, never acceptance."""
...
fields={'model':'qwen-image-2.1','prompt':'Return the input image unchanged.','n':1,'size':'1024x1024','num_inference_steps':40,'guidance_scale':1.0,'true_cfg_scale':1.0,'seed':42,'generator_device':'cpu','output_format':'png','response_format':'b64_json','background':'opaque','enable_teacache':False,'perf_dump_path':str(out/'perf.json')}
(out/'request.json').write_text(json.dumps(fields,indent=2)+'\n')
body,ctype=n.multipart(fields,raw,'h003identity')
m={'id':'D03','http_calls':0,'completed_outputs':0,'steps_completed':None,'status':'prepared'}

...
def main(payload):
    ...
    r = m.Runtime()
    def unit():
        keys = ['MainPID', 'ActiveState', 'SubState', 'Result', 'ExecMainCode', 'ExecMainStatus', 'ExecMainStartTimestamp', 'ControlGroup', 'Restart']
    ...
    api = unit()
    assert api['ActiveState'] == 'active' and int(api['MainPID']) > 0 and api['Restart'] == 'no'
    old_models = models()
    ...
    receipt_saved = False
    try:
        # No lifecycle lease is held during drain, so existing recovery cannot
        # contend with this helper. SIGTERM MAIN avoids systemd stop-timeout KILL.
        assert unit() == api
        m.run(['systemctl', 'kill', '--kill-whom=main', '--signal=SIGTERM', 'llm-image-api.service'], timeout=5)
        receipt['signal_sent'] = 'SIGTERM_MAIN'
        deadline = time.monotonic() + 30
        after = unit()
        while after['MainPID'] != '0' and time.monotonic() < deadline:
            time.sleep(.25)
            after = unit()
        ...
        # Acquire canonical lease ONLY after clean exit; recheck all identities.
        with m.acquire_lease(blocking=False):
            ...
            receipt.update(api_after=after, backend_after=backend, models_after=old_models,
                           ...
                           no_lease_during_drain=True, canonical_lease_after_exit=True,
                           clean_exit_and_identity_verified=True, clean_stop_proved=True)
            with r.anchor() as anchor:
                anchor.atomic_json('receipts/H003-CAPACITY-WINDOW.json', receipt)
            receipt_saved = True
    except BaseException as error:
        receipt['error_type'] = type(error).__name__
        receipt['clean_stop_proved'] = False
    finally:
        # No automatic restart: final reviewed activation uses the one reload.
        try:
            if not receipt_saved:
                with m.acquire_lease(blocking=False), r.anchor() as anchor:
                    anchor.atomic_json('receipts/H003-CAPACITY-WINDOW.json', receipt)
        finally:

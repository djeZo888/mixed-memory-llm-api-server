...
scripts/common/require-data-mounted.sh > "$RUN/evidence/diag-pre-mount.txt"
scripts/common/root-disk-guard.sh --report "$RUN/evidence/diag-pre-root-guard.md"
...
sudo -n docker run --rm --pull never --runtime runc \
  ...
  --mount type=bind,source=/data/build/f1d-qwen-20260915/diagnose-auth-gate.py,target=/diagnose-auth-gate.py,readonly \
  --workdir /fixture --entrypoint python3 \
  sha256:5027e95bf6ec536856b1b52a91d1f35ff5c564ab83e8a94758a169ff09bb8df3 \
  /diagnose-auth-gate.py \
  > "$RUN/evidence/auth-diagnostic.stdout" 2> "$RUN/evidence/auth-diagnostic.stderr"
AUTH_EXIT=$?
set -e
printf '%s\n' "$AUTH_EXIT" > "$RUN/evidence/auth-diagnostic.exit"
scripts/common/require-data-mounted.sh > "$RUN/evidence/diag-post-mount.txt"
scripts/common/root-disk-guard.sh --report "$RUN/evidence/diag-post-root-guard.md"
cat "$RUN/evidence/auth-diagnostic.stdout"

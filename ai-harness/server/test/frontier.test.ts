...
async function fixture(
  ...
  return {
    ...
    complete,
    close,
    holdCounts: (value: boolean) => { holdCounts = value; },
    setCount: (v: any) => {
      countResponse = v;
    },
    countResponse,
...
test("exact rendered-token contract, fixed identity/high thinking, dynamic output and protected key", async (t) => {
  const f = await fixture();
  t.after(f.close);
  f.setCount({ ...f.countResponse, count: 127990 });
  const body = {
  ...
  assert.equal([...f.records.values()][0]!.promptTokens, 127990);
});
...
test("480K configured context is distinct from the round 16K occupied-input admission boundary", async (t) => {
  const f = await fixture({ contextWindow: 480000, maxPromptTokens: 16000 });
  t.after(f.close);
  f.setCount({ ...f.countResponse, context_limit: 480000, count: 16001 });
  assert.equal((await f.send()).status, 413);
  assert.equal(f.requests.length, 0);
  f.setCount({ ...f.countResponse, context_limit: 480000, count: 16000 });
  const p = f.send({ max_tokens: 999999 });
  await until(() => f.requests.length === 1);
  assert.equal(f.requests[0]!.body.max_tokens, 65536);
  f.complete(0);
  assert.equal((await p).status, 200);
});
...
test("count endpoint timeout, connection failure, malformed and oversized responses cannot start inference", async t => {
  const f = await fixture({countTimeoutMs:25}); t.after(f.close);
  f.holdCounts(true); assert.equal((await f.send()).status,503);
  f.holdCounts(false);
  for (const value of [undefined,{...f.countResponse,unexpected:"x".repeat(20000)}]) {
    f.setCount(value); assert.equal((await f.send()).status,503);
  }
  assert.equal(f.requests.length,0); assert.equal(f.gateway.frontierSnapshot().state,"idle");
  const missing = await fixture({fixtureUrl:"http://127.0.0.1:1/v1"}); t.after(missing.close);
  assert.equal((await missing.send()).status,503); assert.equal(missing.requests.length,0);
  const qwen = missing.send({},undefined,true); await until(()=>missing.requests.length===1);
  missing.complete(0); assert.equal((await qwen).status,200);
});

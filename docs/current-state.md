...
- Project state: M0-M9E have been attempted, with M0-M9D merged into `main`, M9E stopped on branch `milestone/m9e-large-model-poc`, M9E-R1 stopped on branch `milestone/m9e-r1-minimax-runtime-remediation`, and M9E-R2 stopped on branch `milestone/m9e-r2-sm120-minimax-remediation`. M9F is the current planning branch `milestone/m9f-offline-resilience-mixed-memory-plan` and reframes the project around offline/local AI resilience, mixed RAM/VRAM large-model inference, local memory/RAG, and model-role routing. MiniMax-M3-MXFP8 remains downloaded at `/data/models/minimax-m3-mxfp8` (`414G`) but is parked because the released KTransformers/SGLang-KT MiniMax MXFP8 path is SM90/SM100-oriented and lacks a clean SM120 path on this RTX PRO 6000 Blackwell Workstation VM. The active backend remains `Qwen/Qwen3-30B-A3B-Instruct-2507` on SGLang at `http://127.0.0.1:30001/v1`, bound to `127.0.0.1` only. The next investigated large mixed-memory candidate is `Qwen/Qwen3.5-397B-A17B-FP8`, but no Qwen3.5 model has been downloaded. Local memory/RAG is now a core roadmap feature. Public API exposure is still not configured and remains deferred until after local model function, memory, and routing are robust.
...
- M9F offline-resilience mixed-memory architecture plan: planning branch `milestone/m9f-offline-resilience-mixed-memory-plan`; report is `reports/m9f-offline-resilience-mixed-memory-plan.md`. M9F records offline resilience as the primary goal, adds model-role docs, adds memory/RAG architecture docs, selects `Qwen/Qwen3.5-397B-A17B-FP8` for M9G runtime preflight, and keeps download/build/deploy stopped until human review. The 30B backend remained running; no Qwen3.5 model download, Docker image pull, runtime build/install, public API exposure, or Docker/containerd daemon change occurred.
...
- M9D merged into `main` with merge commit `3ae263395fe1c5bca260a0136ab77a6f18110bcb`.
- M9D main merge report: `reports/m9d-main-merge.md`.
- Pre-M9E handoff: `docs/pre-m9e-large-model-poc-handoff.md`.
...
- Selected first M9E proof-of-life target: `MiniMaxAI/MiniMax-M3-MXFP8`.
- Selected M9E runtime path: KTransformers / KT-Kernel plus SGLang heterogeneous CPU/GPU serving.
...
- `docs/large-model-feasibility.md` if present
